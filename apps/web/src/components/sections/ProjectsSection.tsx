...
function ModalIconButton({
  ...
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className="inline-flex h-10 w-full items-center justify-center gap-2 rounded-[0.6rem] border border-[rgba(181,138,221,0.34)] bg-[rgba(16,16,21,0.78)] px-3 text-[0.64rem] font-medium tracking-[0.06em] text-[var(--color-accent-soft)] transition duration-300 hover:-translate-y-0.5 hover:border-[rgba(181,138,221,0.62)] hover:bg-[rgba(30,20,42,0.98)] hover:text-white hover:shadow-[0_14px_28px_rgba(61,24,92,0.26)]"
    >
      {children}
      <span>{label}</span>
    </a>
...
export function ProjectsSection({ projects, state }: ProjectsSectionProps) {
  ...
  const modal =
    selectedProject && typeof document !== "undefined"
      ? createPortal(
          <div
            className="fixed inset-0 z-[999] overflow-y-auto bg-[rgba(8,7,10,0.94)] backdrop-blur-md"
          ...
          >
            <div className="flex min-h-full items-center justify-center p-3 sm:p-6">
              <div
                ...
                className="relative w-full max-w-6xl overflow-hidden border border-white/12 bg-[linear-gradient(180deg,rgba(14,13,18,0.985)_0%,rgba(10,10,13,0.985)_100%)] shadow-[0_30px_100px_rgba(0,0,0,0.45)] max-lg:max-w-[min(92vw,30rem)] max-lg:max-h-[calc(100dvh-1.5rem)] lg:max-h-[calc(100dvh-3rem)]"
              ...
              >
                ...
                <div className="grid overflow-y-auto max-lg:max-h-[calc(100dvh-1.5rem)] lg:max-h-[calc(100dvh-3rem)] lg:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]">
                  <div className="relative min-h-[190px] border-b border-white/8 bg-[rgba(14,12,18,0.92)] sm:min-h-[300px] lg:min-h-0 lg:self-stretch lg:border-b-0 lg:border-r lg:border-white/8">
                    {selectedPreview ? (
                      <Image
                        src={selectedPreview}
                  ...
                  <div className="flex flex-col p-5 sm:p-8 lg:p-10">
                    ...
                    <div className="mt-5 inline-flex w-fit items-center gap-2 border border-[rgba(181,138,221,0.24)] bg-[rgba(22,18,29,0.7)] px-3 py-2.5">
                    ...
                    <h3 className="type-heading-display mt-6 text-[1.56rem] leading-[0.96] sm:mt-7 sm:text-[2.15rem] lg:text-[2.45rem]">
                      {selectedProject.title}
                    </h3>

                    <p className="mt-3.5 max-w-[28rem] text-[0.78rem] leading-[1.62] text-[rgba(217,210,225,0.76)] sm:mt-4 sm:text-[0.86rem] sm:leading-[1.75]">
                    ...
                    <div
                      className={`mt-6 grid gap-3 sm:mt-8 ${
                        selectedProject.projectUrl ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1"
                      }`}
                    >
